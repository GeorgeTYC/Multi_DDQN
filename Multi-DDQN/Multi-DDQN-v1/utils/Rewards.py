from base.BaseRewards import BaseRewards
...
class Rewards(object):
    ...
    def is_mapped_single_vnf(self, state: State, actions: Actions):
        ...
        if nodes_cpu.shape[0] != action.shape[0] or vnfs_matrix.shape[0] != sfc_request.shape[0]:
            raise ValueError(
                "The number of rows in arrays nodes_cpu and sfc_request, and in arrays vnfs_matrix and action must match")
        if vnfs_supported.shape[0] != action.shape[0] or vnfs_supported.shape[1] != sfc_request.shape[0]:
            raise ValueError(
    ...
    def calculate_reward_V2(self, state: State, actions: Actions, topo: Topology):
        ...
        if order < self.vnfs_num_per_sfc - 1:
            ...
            reward, state_new, topo_new = self.calculate_reward_single_vnf_with_links(state, actions, topo)
            pass
    ...
    def calculate_reward_single_vnf(self, state: State, actions: Actions, topo: Topology):
        ...
        # 更新网络中的CPU资源
        state_new.nodes_cpu = nodes_cpu
        topo_new.nodes_cpu = nodes_cpu
    ...
    def calculate_reward_single_vnf_with_links(self, state:State, actions: Actions, topo: Topology):
        ...
        reward_link, state_new2, topo_new2 = self.calculate_reward_links(state=state_new, actions=actions, topo=topo_new)

        return reward_vnf+reward_link, state_new2, topo_new2

    def calculate_reward_links(self, state: State, actions: Actions, topo: Topology):
        # 计算映射链路的奖励
        ...
        # 更新网络中的带宽资源
        remaining_bandwidth = topo.links_bandwidth
        links_distance = topo.links_distance
        remaining_bandwidth = self.update_remaining_bandwidth(remaining_bandwidth=remaining_bandwidth, links_distance=links_distance, nodes_list=action_new, bandwidth_request=bandwidth_request)
        topo_new.links_bandwidth = remaining_bandwidth

        state_new.nodes_bandwidth = state_new.calculate_bandwidth(remaining_bandwidth)
        state_new.nodes_distance, state_new.bandwidth_bottleneck = state_new.floyd_warshall_with_bandwidth(topo_new.links_distance, remaining_bandwidth)

        # 计算相对时延
        nodes_distance = state.nodes_distance
        total_distance = self.get_total_distance(action_new, nodes_distance=nodes_distance)

        row_index = np.argmax(ingress_node)
        col_index = np.argmax(egress_node)

        reward_temp += (nodes_distance[row_index,col_index] / total_distance)

        if reward_temp > 0.1:
            reward_link = reward_temp

        return reward_link, state_new, topo_new
